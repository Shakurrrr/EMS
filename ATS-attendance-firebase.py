...
def firebase_init():
    ...
    if not FIREBASE_ENABLED or _firebase_inited:
        return
    if not os.path.exists(SERVICE_ACCOUNT):
        print(f"[FIREBASE] Service account missing: {SERVICE_ACCOUNT}")
        return
    cred = credentials.Certificate(SERVICE_ACCOUNT)
    firebase_admin.initialize_app(cred, {"storageBucket": FIREBASE_BUCKET})
    _firebase_inited = True
    print("[FIREBASE] Initialized Admin SDK")

def firebase_upload_pdf(local_path: str, storage_path: str):
    ...
    if not os.path.exists(local_path):
        print(f"[FIREBASE] File missing, skip upload: {local_path}")
        return
...
def upload_daily_report_to_firebase(target_day: date = None):
    """Uploads daily_YYYY-MM-DD.pdf to Firebase Storage once at 7PM."""
    try:
        d = target_day or _today()
        local_pdf = os.path.join(REPORTS_DIR, f"daily_{d.isoformat()}.pdf")
        storage_path = f"reports/daily/{d.isoformat()}.pdf"
...
def upload_weekly_report_to_firebase(any_day: date = None):
    ...
    try:
        ...
        # local weekly file your system generates:
        local_pdf = os.path.join(REPORTS_DIR, f"weekly_{iso_year}-{iso_week:02d}.pdf")

        # Android expects: YYYY-Www.pdf
        storage_pdf_name = f"{iso_year}-W{iso_week:02d}.pdf"
        storage_path = f"reports/weekly/{storage_pdf_name}"

        firebase_upload_pdf(local_pdf, storage_path)
    except Exception as e:
        print(f"[FIREBASE] Weekly scheduled upload failed: {e}")
...
def _register_scheduler_jobs(scheduler, mgr):
    ...
    scheduler.add_job(lambda: upload_daily_report_to_firebase(_today()),
                      'cron', hour=19, minute=0)
                      
    scheduler.add_job(lambda: upload_weekly_report_to_firebase(_today()),
                      'cron', day_of_week='fri', hour=19, minute=0)
